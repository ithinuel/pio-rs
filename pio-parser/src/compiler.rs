...
impl<'i> Reify<'i> for Instruction<'i> {
    fn reify(&mut self, prog_name: &'i str, defines: &Defines<'i>) -> Result<(), Error<'i>> {
        let mut pending = Vec::new();
        if let Some(delay) = self.delay.take() {
            self.delay = Some(Expression::Value(Value::Integer(delay.resolve(
                SymbolId {
                    prog_name,
                    var_name: "delay",
                },
        ...
        if let Some(side_set) = self.side_set.take() {
            self.side_set = Some(Value::Integer(side_set.resolve(
                SymbolId {
                    prog_name,
                    var_name: "side_set",
                },
...
impl<'i> Reify<'i> for InstructionOps<'i> {
    fn reify(&mut self, prog_name: &'i str, defines: &Defines<'i>) -> Result<(), Error<'i>> {
        let mut pending = Vec::new();
        match self {
            InstructionOps::Push { .. }
            ...
            | InstructionOps::Nop
            | InstructionOps::Mov { .. } => {}
            InstructionOps::Out { bit_count: v, .. }
            | InstructionOps::In { bit_count: v, .. }
            | InstructionOps::Wait { duration: v, .. }
            | InstructionOps::Irq { value: v, .. }
            | InstructionOps::Set { value: v, .. } => {
...
#[cfg(test)]
mod tests {
    use crate::ast::{Expression, Instruction, InstructionOps, Value};
